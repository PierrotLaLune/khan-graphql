...
export class KhanApi {
  ...
  constructor() {
    ...
    this.instance.defaults.jar = cookieJar
  }
  ...
  axios = (config: AxiosRequestConfig) => this.instance(config)
  ...
  graphQL = async (endpoint: string, payload: any, method = "POST") => {
  ...
  getStudentsList = async ({
    ...
    const payload = {
      ...
      variables: {
        ...
        after: 0,
        pageSize,
        teacherKaid: teacherKaid || this.user?.kaid,
      },
      query:
  ...
  ClassSubjectMasteryProgress = async ({
    classId,
    topicId,
    teacherKaid,
  }: {
  ...
  getUserInfoForSubjectProgress = async ({ topicId }: { topicId: string }) => {
    const payload = {
      operationName: "getUserInfoForSubjectProgress",
      variables: { topicId },
      query:
